...
// ─────────────────────────────────────────────
class LargeScreenNegotiationChatScreen extends StatefulWidget {
  const LargeScreenNegotiationChatScreen({
    super.key,
  ...
  @override
  State<LargeScreenNegotiationChatScreen> createState() => _NegotiationChatScreenState();
}

class _NegotiationChatScreenState extends State<LargeScreenNegotiationChatScreen> {
  late NegotiationChatRouteData _selectedRouteData;
  late List<NegotiationChatRouteData> _conversationOptions;

  @override
  void initState() {
    super.initState();
    _selectedRouteData = widget.routeData;
    _conversationOptions = _buildConversationOptions(widget.routeData);
  }

  @override
  void didUpdateWidget(covariant LargeScreenNegotiationChatScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.routeData.product.id != widget.routeData.product.id) {
      _selectedRouteData = widget.routeData;
      _conversationOptions = _buildConversationOptions(widget.routeData);
    }
  }

  List<NegotiationChatRouteData> _buildConversationOptions(
    NegotiationChatRouteData seed,
  ) {
    final products = InMemorySeedData.buildProducts();
    final orderedProducts = <ProductModel>[
      seed.product,
      ...products.where((product) => product.id != seed.product.id).take(4),
    ];

    return orderedProducts
        .map(
          (product) => NegotiationChatRouteData(
            product: product,
            currentUser: seed.currentUser,
            seller: product.id == seed.product.id ? seed.seller : null,
            viewerRole: seed.viewerRole,
          ),
        )
        .toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final routeData = _selectedRouteData;
    final seller =
        routeData.seller ??
    ...
    final useSplitView = !Responsive.isMobile(context);

    return ChangeNotifierProvider<NegotiationChatController>(
      key: ValueKey('${routeData.product.id}-${currentUser.id}'),
      ...
      child: Builder(
        builder: (context) {
          final detailView = KeyedSubtree(
            key: ValueKey(routeData.product.id),
            child: _NegotiationChatView(
              showBackButton: widget.showBackButton,
              embedded: useSplitView,
            ),
          );
          ...
          return _DesktopConversationShell(
            conversations: _conversationOptions,
            selectedRouteData: routeData,
            showBackButton: widget.showBackButton,
            onSelected: (selected) {
              if (selected.product.id == _selectedRouteData.product.id) {
                return;
              }
              setState(() => _selectedRouteData = selected);
            },
            child: detailView,
          );
        },
...
class _NegotiationChatViewState extends State<_NegotiationChatView> {
  ...
  Widget build(BuildContext context) {
        ...
        Expanded(
          child: controller.isInitializing
              ...
              : _MessageList(
                  ...
                  onOpenOfferSheet: () => _showOfferSheet(context, controller),
                ),
        ),
        _BeautifulInputBar(
          ...
          includeBottomSafeArea: !widget.embedded,
        ),
      ],
    );
    ...
    final content = Column(
      children: [
        _ConversationHeader(
          ...
          thread: thread,
          showBackButton: widget.showBackButton && !widget.embedded,
        ),
    ...
    if (widget.embedded) {
      return DecoratedBox(
        decoration: const BoxDecoration(color: _kDarkBg),
        child: content,
      );
    }
...
// Desktop shell
// ─────────────────────────────────────────────
class _DesktopConversationShell extends StatelessWidget {
  const _DesktopConversationShell({
    required this.conversations,
    required this.selectedRouteData,
    required this.showBackButton,
    required this.onSelected,
    required this.child,
  });

  final List<NegotiationChatRouteData> conversations;
  final NegotiationChatRouteData selectedRouteData;
  final bool showBackButton;
  final ValueChanged<NegotiationChatRouteData> onSelected;
  final Widget child;
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text(
                        'Review active offers and continue checkout from the same workspace.',
                        style: TextStyle(
                          color: _kTextSoft,
                          fontSize: 13,
                          height: 1.45,
                ...
                Expanded(
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _ConversationPreviewTile(
                        routeData: conversation,
                        isSelected:
                            conversation.product.id == selectedRouteData.product.id,
...
class _ConversationPreviewTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final product = routeData.product;
    final seller = routeData.seller ?? InMemorySeedData.sellerForProduct(product);
    final preview = product.description.trim().isEmpty
    ...
    return InkWell(
      ...
      child: Ink(
        ...
        decoration: BoxDecoration(
          color: isSelected ? _kCard : _kCardDeep,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
            color: isSelected
                ? _kTealLight.withValues(alpha: 0.5)
                : _kBorder,
          ),
        ),
        child: Row(
                  ...
                  Row(
                    children: [
                      ...
                      Text(
                        'Offer',
                        style: TextStyle(
                          color: isSelected ? _kTealLight : _kTextSoft,
                          fontSize: 11.5,
                          fontWeight: FontWeight.w700,
                        ),
                      ),
